...
> *User Story Map diagram will be added here.*
## Epics
Этапы User Journey и их ценность  
1. *Управление траекторией*: пользователь может осознанно спланировать свое развитие и выстроить понятную структуру навыков вместо хаотичных попыток обучения.
3. *Управление действиями и ресурсами*: пользователь экономит время и когнитивные ресурсы, сохраняя проверенные материалы и шаги в одном месте.
4. *Использование траектории*: пользователь быстро ориентируется в своей карте развития и понимает, что именно делать дальше для развития конкретного навыка.
5. *Регулярная практика*: пользователь поддерживает устойчивый прогресс за счет фиксации выполненных действий и ощущения движения вперед, даже при высокой занятости.
## User Stories List
...
Scope MVP – истории с приоритетом Must. Истории Should рассматриваются как возможное расширение MVP, остальные – вне Scope.  
...
- **US010. Получение и настройка уведомлений** – Как пользователь я хочу настраивать и получать уведомления о необходимости продолжить обучение, чтобы поддерживать регулярную практику и формировать привычку. **Priority:**	С  
...
## Scope MVP
В MVP входят следующие пользовательские истории:
